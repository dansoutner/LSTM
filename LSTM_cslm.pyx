...
class LSTM:
	...
	def index_to_vector_cslm(self, int idx, int dic_len):
		o = np.zeros((dic_len+len(self.projections["%hesitation"])))
		
		if idx > -1:
			o[idx] = 1.
			try:
				prj = np.array(self.projections[self.dic[idx]])
			except KeyError:
				prj = np.zeros(len(self.projections["%hesitation"]))
			o[dic_len:] = prj
		#print o
		return o
	...
	def ppl(self, np.ndarray[long, ndim=1] iText):
		...
		for ii in range(len(iText)-1):
			if iText[ii + 1] > -1:
				o = self.forward(self.index_to_vector_cslm(iText[ii], len(self.dic)))
	...
	def __init__(self):
		...
		self.runTrain(iText, iTest, rnd_seed, iHidden, len(self.dic)+80, len(self.dic))
